...
export class DisplayEventComponent {
 ...
 //stand: any;
  ...
  constructor(private reservationService: ReservationsService,private route: ActivatedRoute, private router: Router, private http: HttpClient,
    private getevent:AboutService,private login:LoginService , private photoservice: PhotoService) {}
...
fetchReviews() {
  this.photoservice.getEventReviews().subscribe((res: any[]) => {
    this.ratings = res;
    console.log(res);
  });
}

getReviewsForEvent(eventId: number) {
  return this.ratings.filter(rating => rating.event_id === eventId);
}
...
getEventRatings(eventId: number): any[] {
  return this.ratings.filter(r => r.event.id_event === eventId);
}
